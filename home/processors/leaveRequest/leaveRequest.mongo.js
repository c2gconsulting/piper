...
db.getModel('leaverequestcontext', function(err, model) {
    if (err) {
        logger.error('Fatal error: ' + e + '. Cannot retrieve leave context');
    } else {
        LeaveRequest = model;
    }   
});
...
logger.info("Processor: REQUEST_LEAVE");
    ...
    try {
        ...
        LeaveRequest.findOneAndUpdate(
            {slackHandle:client.slackHandle , username:user.name}, JSON.parse(updates), {upsert: true}, function (err, leaveContext) {
            if (err) {
                ...
                var convoDepth = leaveContext.conversationDepth ? (leaveContext.conversationDepth + 1) : 1;
                
                //Update the conversation depth by the side.. even if it fails no issues
                LeaveRequest.update( {_id: leaveContext._id}, { $set: {conversationDepth: convoDepth}}, function(ex2, result) {
                ...
                if ((!leaveContext.leaveType) && (continueValidation)){
                    ...
                    LeaveRequest.update( {_id: leaveContext._id}, { $set: {lastDemand: "LEAVE_TYPE"}}, function(ex2, result) {
                ...
                if ((!leaveContext.startDate)&& (continueValidation)){
                    ...
                    if ((leaveContext.genericDate) && (leaveContext.lastDemand == 'START_DATE')){ //check if start date is implied by last demand
                        ...
                        LeaveRequest.update( {_id: leaveContext._id}, { $set: {startDate: leaveContext.startDate, genericDate: ""}}, function(ex2, result) {
                        ...
                        //Update start date by the side.. even if it fails no issues
                        LeaveRequest.update( {_id: leaveContext._id}, { $set: {startDate: leaveContext.startDate}}, function(ex2, result) {
                            ex2 ? logger.error('Error updating start date: ' + ex2) : logger.info('Start date set to: ' + leaveContext.startDate);
                        });
                        ...
                        //Update last demand by the side.. even if  it fails no issues
                        LeaveRequest.update( {_id: leaveContext._id}, { $set: {lastDemand: "START_DATE"}}, function(ex2, result) {
                ...
                if ((!leaveContext.endDate)&& (continueValidation)){
                    ...
                    if ((leaveContext.genericDate) && (leaveContext.lastDemand == 'END_DATE')){ 
                        ...
                        LeaveRequest.update( {_id: leaveContext._id}, { $set: {endDate: leaveContext.endDate , genericDate: ""}}, function(ex2, result) {
                        ...
                        ///Update end date by the side.. even if it fails no issues
                        LeaveRequest.update( {_id: leaveContext._id}, { $set: {endDate: leaveContext.endDate }}, function(ex2, result) {
                            ex2 ? logger.error('Error updating end date: ' + ex2) : logger.info('End date set to: ' + leaveContext.endDate);
                        });
                        ...
                        //Update last demand by the side.. even if it fails no issues
                        LeaveRequest.update( {_id: leaveContext._id}, { $set: {lastDemand: "END_DATE"}}, function(ex2, result) {
                ...
                if (continueValidation){
                    ...
                    slackCallback('', "Ok, lets see if we can create a " + leaveContext.leaveType + " leave starting on " + leaveContext.startDate + " and ending on " + leaveContext.endDate);
